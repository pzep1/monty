...
pub enum MountError {
    ...
    OverlappingMounts { virtual_path: String, host_path: std::path::PathBuf, existing_virtual_path: String, existing_host_path: std::path::PathBuf },
...
overlap check in [`MountTable::push_mount`](#mounttable); operations never resolve
through it.
...
at most once per table, see [`MountTable::push_mount`](#mounttable); cloning shares the
descriptor.
...
Returns [`MountError::OverlappingMounts`](#mounterror) if the mount overlaps an
already-registered one (see [`Self::push_mount`](#mounttable)).
...
<div class="rust-decl-links" data-links="Mount #mount;MountError #mounterror" hidden></div>
...
Returns [`MountError::OverlappingMounts`](#mounterror) if the mount's host directory
